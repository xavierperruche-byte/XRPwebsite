...
function getBackgroundIgnoreLockConflict(flags) {
  if (!flags.background) {
    return null;
  }
  return [
...
async function dev({ flags }) {
  ...
  const wantsBackground = !!flags.background || supportsAgentAutoBackgrounding(process.platform) && agentDetected && !ignoreLock;
